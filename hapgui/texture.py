...
class getMouEvt():
    ...
    def getMouseEvent(self):
        ...
        # algorithm: compute the event data to arduino commend
        try:
            self.ser.write(str.encode('w'))
        except NameError:
            print ('Send "w" to serial')
        except AttributeError:
            print ('Send "w" to serial')

        if ((self.distcount//200) != ((self.distcount-self.mevt.vxy[0])//200)):
            print('click')
            #print (distcount%200)
            #print ((distcount-x)%200)
            try:
